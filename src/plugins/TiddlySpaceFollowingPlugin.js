...
var followMacro = config.macros.followTiddlers = {
	...
	handler: function(place, macroName, params, wikifier, paramString, tiddler) {
		...
		this.getFollowers(function(followers) {
			...
			} catch(e) {} // ajax request might fail
		}, user);
	},
	...
	followingOnClick: function(ev, list) {
		...
		if(!place) {
			...
			if($(place).attr("openedby") == "followTiddlers") {
				...
				$(place).attr("openedby","").empty().append(contentEl).slideUp(500);
			} else {
	...
	getFollowers: function(callback, username) { // returns a list of spaces being followed by the existing space
		var adaptor = store.getTiddlers()[0].getAdaptor();
		var followersCallback = function(user) {
			...
			if(!user.anon) {
				...
				if(username == tiddlyspace.currentSpace.name) { // just get the tiddlers in the local store
					...
					callback(followers);
				} else { // request them using cors if you can
					if(followMacro.follower_names_cache[username]) { // use cached list to save ajax requests
...
headers: {"X-ControlView": "false"}
						...
						alert(e);
						callback([], true); // run callback on an empty list
					}
				}
			}
		};
...
var followersMacro = config.macros.followers = {
	...
	handler: function(place, macroName, params, wikifier, paramString, tiddler) {
		...
		var container = $('<div class="followers" />').text(locale.pleaseWait).appendTo(place)[0];
		var followersCallback = function(user) {
			if(user.anon) {
...
var followingMacro = config.macros.following = {
	...
	handler: function(place, macroName, params, wikifier, paramString, tiddler) {
		...
		var fat = args.fat ? "&fat=y" : "";
		var username = params[0] || false;
		var container = $('<div class="following" />').text(locale.pleaseWait).appendTo(place)[0];
